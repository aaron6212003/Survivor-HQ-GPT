...
export default function ScoreTicker(){
 ...
 return <div className="score-ticker" aria-label="NFL score ticker"><div className="score-ticker__label"><span className="score-ticker__dot"/> {label}</div><div className="score-ticker__track">{featured.length?featured.map(game=><div className="score-ticker__game" key={game.id}><span>{game.away_team?.code}</span><strong>{game.away_score ?? 0}</strong><span className="score-ticker__at">@</span><span>{game.home_team?.code}</span><strong>{game.home_score ?? 0}</strong><em className={game.status==='in_progress'?'is-live':''}>{game.status==='in_progress'?'LIVE':game.status==='final'?'FINAL':game.status_detail||'UPCOMING'}</em></div>):<span className="text-slate-500">Scores will appear here when the NFL schedule is available.</span>}</div></div>;
}
