...
export async function POST(req:Request){
 try{
  ...
  if(!['image/jpeg','image/png','image/webp'].includes(image.type))throw new Error('Choose a standard photo from your camera roll.');
  if(image.size>MAX_BYTES)throw new Error('Choose an image smaller than 10 MB.');
