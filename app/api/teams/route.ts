...
export async function GET() {
  ...
  const teams = getAllTeams(); const games = getAllGames(); const standings = await getNflStandings();
  ...
  const response = teams.map(team => ({ ...team, ...(standings.get(team.name.toLowerCase()) || standings.get(team.code.toLowerCase()) || records.get(team.id) || {}) })); cache.hqTeamsCache = { at: Date.now(), teams: response };
