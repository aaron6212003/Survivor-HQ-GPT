...
function isComplete(snapshot) { return Array.isArray(snapshot?.games)&&snapshot.games.length>0&&snapshot.games.every(game=>['final','canceled'].includes(game.status)); }
export async function activeWeek(client,league,meta) {
 const base=Math.max(league.resumeWeek||1,league.startWeek,league.season<meta.season?18:league.season>meta.season?league.startWeek:meta.week);
 if(league.season!==meta.season) return base;
 // ESPN can move its labeled week before Monday night finishes, or a little after it does.
 // The actual schedule decides the league's active pick week.
 const current=await nflWeek(client,league.season,base);
 if(isComplete(current)&&base<18) return base+1;
 if(base>league.startWeek) { const previous=await nflWeek(client,league.season,base-1); if(!isComplete(previous)) return base-1; }
 return base;
}
export async function leagueSchedules(client,league,meta,selectedWeek) {
 const active=await activeWeek(client,league,meta);const weeks=[...new Set([...Array(active-league.startWeek+1)].map((_,i)=>league.startWeek+i).concat(selectedWeek))];
 const snapshots=[];
