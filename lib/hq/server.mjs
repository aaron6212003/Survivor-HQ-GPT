...
export async function hqAction(client,user,input) {
 ...
 for(let attempt=0;attempt<5;attempt++) {
  ...
  if(action==='matrix') {
   ...
   return {leagueId:league.id,activeWeek:active,lives:{alive:entries.length,total:managed.length},entries:entries.map(e=>({id:e.id,name:e.name,entryNumber:e.entryNumber,alive:e.alive})),schedules,picks:league.picks};
  }
  if(action==='archive') {
