...
import {avatars,label,integer,settings,fail,createLeague,addEntry,savePicks,viewLeague,normalizeLeagueConfig,standings} from './rules.mjs';
import {currentNfl,nflWeek,leagueSchedules,activeWeek} from './espn.mjs';
import {proAccess,profileCustomization,saveProfileCustomization,setDeveloperOverride} from './pro.mjs';
const teamCodes=new Set(['ARI','ATL','BAL','BUF','CAR','CHI','CIN','CLE','DAL','DEN','DET','GB','HOU','IND','JAX','KC','LV','LAC','LAR','MIA','MIN','NE','NO','NYG','NYJ','PHI','PIT','SEA','SF','TB','TEN','WAS','WSH']);
...
async function summary(client,row,user,meta) {
 const s=row.state;return {id:row.id,code:row.code,name:s.name,season:s.season,format:s.config.format,memberCount:s.members.length,entryCount:s.entries.filter(e=>e.userId===user.id).length,owner:s.ownerId===user.id,coOwner:!!((s.coOwnerOf||{})[user.id]||(s.coOwnerIds||[]).includes(user.id)),activeWeek:await activeWeek(client,s,meta),archived:s.archived};
}
...
export async function hqAction(client,user,input) {
 const action=input.action||'bootstrap';const profile=await profileFor(client,user);const pro=await proAccess(client,user);
 if(action==='proOverride') return {pro:await setDeveloperOverride(client,user,input.enabled)};
 if(action==='profileCustomization') return {customization:await saveProfileCustomization(client,user,input,pro),pro};
 if(action==='profile') {
  if(!avatars.includes(input.avatar)&&!String(input.avatar||'').startsWith(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/profile-photos/${user.id}/`)&&!/^https:\/\/a\.espncdn\.com\/i\/teamlogos\/nfl\/500\/[a-z]{2,3}\.png$/.test(String(input.avatar||''))) fail('Choose an available avatar.');
  ...
  const row={user_id:user.id,display_name:label(input.displayName,'Display name',40),avatar:input.avatar,favorite_team:favoriteTeam,jersey_number:null,bio:null,updated_at:new Date().toISOString()};
 ...
 try {meta=await currentNfl();} catch(error) {
  if(action==='bootstrap')return {profile,user:{id:user.id,email:user.email},meta:null,leagues:await Promise.all((await memberLeagues(client,user)).map(async r=>({...await summary(client,r,user,{season:r.state.season,week:r.state.startWeek}),activeWeek:null}))),pro,customization:await profileCustomization(client,user),warning:'ESPN is unavailable. Your account and saved leagues are still accessible.'};
  throw error;
 }
 if(action==='bootstrap') return {profile,user:{id:user.id,email:user.email},meta,leagues:await Promise.all((await memberLeagues(client,user)).map(l=>summary(client,l,user,meta))),pro,customization:await profileCustomization(client,user)};
 ...
 for(let attempt=0;attempt<5;attempt++) {
  ...
  const week=integer(input.week??await activeWeek(client,league,meta),1,18,'Week');
  ...
  if(action==='matrix') {
   const snapshots=await Promise.all(Array.from({length:18},(_,i)=>nflWeek(client,league.season,i+1)));
   const schedules=Object.fromEntries(snapshots.map(s=>[s.week,s.games])); const active=await activeWeek(client,league,meta);
   const managedId=league.ownerId===user.id?user.id:((league.coOwnerOf||{})[user.id]||user.id);
   const managed=league.entries.filter(e=>e.userId===managedId); const standingById=new Map(standings(league,schedules,active).map(item=>[item.id,item]));
   const entries=managed.map(entry=>({...entry,...standingById.get(entry.id)})).filter(entry=>entry.alive);
   const plans=Object.fromEntries(entries.map(entry=>[entry.id,league.matrixPlans?.[`${user.id}:${entry.id}`]||{}]));
   return {leagueId:league.id,activeWeek:active,lives:{alive:entries.length,total:managed.length},entries:entries.map(e=>({id:e.id,name:e.name,entryNumber:e.entryNumber,alive:e.alive})),schedules,picks:league.picks,plans};
  }
  if(action==='saveMatrixPlans') {
   const managedId=league.ownerId===user.id?user.id:((league.coOwnerOf||{})[user.id]||user.id);
   const entry=league.entries.find(item=>item.id===input.entryId&&item.userId===managedId);
   if(!entry) fail('Choose one of your active Survivor entries.');
   if(!input.plans||typeof input.plans!=='object'||Array.isArray(input.plans)) fail('Invalid Matrix plans.');
   const plans={};
   for(const [week,team] of Object.entries(input.plans)) {
    const number=Number(week);const code=String(team);
    if(!Number.isInteger(number)||number<1||number>18||!teamCodes.has(code)) fail('Invalid Matrix plan.');
    plans[number]=code;
   }
   if(new Set(Object.values(plans)).size!==Object.keys(plans).length) fail('Plan each team only once.');
   league.matrixPlans={...(league.matrixPlans||{}),[`${user.id}:${entry.id}`]:plans};
  ...
  if(action!=='saveMatrixPlans') league.audit.push({action,actor:user.id,at:new Date().toISOString()});
  ...
  if(write.data) return action==='saveMatrixPlans'?{plans:league.matrixPlans?.[`${user.id}:${input.entryId}`]||{}}:{league:await publicView(client,league,user,meta,action==='reset'||action==='rollover'?league.startWeek:week)};
 }
