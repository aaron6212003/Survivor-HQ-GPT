...
export async function hqAction(client,user,input) {
 ...
 try {meta=await currentNfl();} catch(error) {
  if(action==='bootstrap')return {profile,user:{id:user.id,email:user.email},meta:null,leagues:(await memberLeagues(client,user)).map(r=>({...summary(r,user,{season:r.state.season,week:r.state.startWeek}),activeWeek:null})),pro,customization:await profileCustomization(client,user),warning:'ESPN is unavailable. Your account and saved leagues are still accessible.'};
  throw error;
 }
 if(action==='bootstrap') return {profile,user:{id:user.id,email:user.email},meta,leagues:(await memberLeagues(client,user)).map(l=>summary(l,user,meta)),pro,customization:await profileCustomization(client,user)};
 ...
 for(let attempt=0;attempt<5;attempt++) {
  ...
  if(action==='matrix') { const snapshots=await Promise.all(Array.from({length:18},(_,i)=>nflWeek(client,league.season,i+1))); return {leagueId:league.id,entries:league.entries.filter(e=>e.userId===(league.ownerId===user.id?user.id:((league.coOwnerOf||{})[user.id]||user.id))).map(e=>({id:e.id,name:e.name,entryNumber:e.entryNumber})),schedules:Object.fromEntries(snapshots.map(s=>[s.week,s.games])),picks:league.picks}; }
  if(action==='archive') {
